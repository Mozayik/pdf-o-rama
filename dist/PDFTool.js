...
let PDFTool =
  (0, _autobindDecorator.default)(
    (_class = class PDFTool {
      ...
      async fields(options) {
        ...
        const catalogDict = this.pdfReader
          ...
          .toPDFDictionary() // Page map is used to get page number from page object ID
        ...
        await this.fs.writeFile(
          options.dataFile,
          _json.default.stringify(fieldData, undefined, "  ")
        )
      }
      ...
      async fill(options) {
        ...
        try {
          data = await _json.default.parse(
            await this.fs.readFile(options.dataFile, {
              encoding: "utf8",
            })
          )
        } catch (e) {
        ...
        for (let i = 0; i < numPages; i++) {
          ...
          const pageModifier = new _hummus.default.PDFPageModifier(
            this.pdfWriter,
            0
          )
          ...
          for (let field of fields) {
            ...
            switch (field.type) {
              ...
              case "signhere":
                if (!font) {
                  throw new Error(
                    "Font file must be specified for signhere fields"
                  )
                }
                ...
                pageContext
                  ...
                  .cm(1, 0, 0, 1, 0, -halfH) // NOTE: The coordinate space of the XObjects is the same as the page!
      ...
      async watermark(options) {
        ...
        const formIDs = this.pdfWriter.createFormXObjectsFromPDF(
          options.watermarkFile,
          _hummus.default.ePDFPageBoxMediaBox
            ...
            if (fieldDictionary.exists("Kids")) {
              ...
              for (let i = 0; i < kidsArray.getLength(); ++i) {
                let widgetDictionary = this.pdfReader
                  ...
                  .toPDFDictionary() // use the dictionary Ap/N dictionary for looking up the appearance stream name
      ...
      parseChoiceValue(fieldDictionary) {
        if (fieldDictionary.exists("V")) {
          ...
          if (
            valueField.getType() == _hummus.default.ePDFObjectLiteralString ||
            valueField.getType() == _hummus.default.ePDFObjectHexString
          ) {
        ...
        switch (fieldType) {
          ...
          case "Tx": {
            ...
            if ((flags >> 25) & 1) {
              ...
              result["plainValue"] = this.parseTextFieldValue(
                fieldDictionary,
                "V"
              )
            } else {
